...
BEGIN
  ...
  FROM (
    SELECT json_build_object(
      ...
      'start_time', to_char(candidate_ts::time, 'HH24:MI'),
      'end_time', to_char(candidate_ts::time + (v_duration || ' minutes')::interval, 'HH24:MI'),
    ...
    -- Gera candidatos a cada 30 minutos dentro do horário de trabalho (usando timestamp para evitar erro)
    CROSS JOIN LATERAL generate_series(
      (d::date + sch.start_time),
      (d::date + sch.end_time) - (v_duration || ' minutes')::interval,
      '30 minutes'::interval
    ) AS candidate_ts
    WHERE
      ...
      AND (d::date > CURRENT_DATE OR candidate_ts::time > (now() AT TIME ZONE 'America/Sao_Paulo')::time + INTERVAL '30 minutes')
      ...
      AND NOT EXISTS (
        ...
        WHERE a.professional_id = p.id
          ...
          AND a.start_time < candidate_ts::time + (v_duration || ' minutes')::interval
          AND a.end_time > candidate_ts::time
      )
      ...
      AND NOT EXISTS (
        ...
        WHERE b.professional_id = p.id
          ...
          AND b.start_time < candidate_ts::time + (v_duration || ' minutes')::interval
          AND b.end_time > candidate_ts::time
      )
    LIMIT p_limit
  ) sub;
